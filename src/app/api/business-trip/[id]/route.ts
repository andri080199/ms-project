...
// Returns a single business trip request with full approval history.
...
// Cancels a business trip request by marking it CANCELLED.
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { id } = await ctx.params;
    const item = await prisma.businessTripRequest.findUnique({ where: { id } });
    if (!item) return NextResponse.json({ success: false, error: 'Tidak ditemukan' }, { status: 404 });
    ...
    await prisma.businessTripRequest.update({ where: { id }, data: { status: 'CANCELLED' } });
